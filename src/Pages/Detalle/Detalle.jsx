...
export default function Detalle() {
  ...
  const {genres, getGenres}=usePeliculas();
  
  const {poster_path, vote_average,original_language ,title, budget, production_companies, status, release_date, overview}= pelicula || {};
  useEffect( ()=>{
    if(movieId){
      getPelicula(movieId);
      getGenres(movieId);
        ...
        <div className="bg-white shadow-lg border-gray-100 max-h-80	 border sm:rounded-3xl p-8 flex space-x-8">
          ...
          <div className="flex flex-col w-1/2 space-y-4">
            ...
            <div>
              ...
              <div className="text-sm text-gray-400"> Generos:
              { genres.map((gene) => (
                  <div className=''>
